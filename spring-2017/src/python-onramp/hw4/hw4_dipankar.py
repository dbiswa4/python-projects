...
convergeThreshold=0.0001 # threshold to check condition of convergence


def filterData(data,targetColumn,columnName=[]): # we need to remove target value from the data frame and need to put x0 values.
    ...
    size=len(dataFrame)
    trainingData= dataFrame[:int(size/2)] # Take first half as training
    trainingLabel=labelFrame[:int(size/2)] #take first half as training label
    testData=dataFrame[int(size/2):] # take second half as test
    testLabel=labelFrame[int(size/2):]
    return trainingData,trainingLabel,testData,testLabel #return target value and modified dataframe
...
def calAccuracy(testLabel,predictLable): # accuracy is percentage of correct prediction made by classifier
  count=0
...
def gradientDescent():
    cost_list=[]
    meanNormalization(data, "target")# second parameter is the name of the target column
...
print("Features selected: ", e_column[1:])
